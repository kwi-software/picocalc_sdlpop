...
Both media use 256-byte snapshots: format magic at offset 0, sequence at 4, valid-key mask at 8, high-score fallback token at 12, 176-byte score payload at 16, 8-byte game save at 192, game-save fallback token at 200, settings fallback token at 204, 8-byte settings payload at 208, reset generation at 216, reset tag at 220, and CRC32 over bytes 0–251 at 252. Numeric fields are little endian. The underlying record size/magic and payload positions remain compatible with the earlier flash journal.
...
Settings payload bytes are version (1), widescreen, vertical filter preference, sound enabled, cheats enabled, status mode (0/1/2), raw LCD brightness and raw keyboard brightness. Startup rejects unsupported versions or invalid values. The existing record magic and sizes are unchanged; valid-key bits 0/1/2 indicate scores/save/settings. No additional flash sector or framebuffer is allocated.

Alt+Delete commits a zero-key snapshot with an incremented reset generation and tag `0x31545352` to flash before attempting SD. A flash reset failure leaves the old data and live settings intact. When reset generations differ, the committed flash reset supersedes the entire stale SD snapshot instead of merging keys. Reads attempt synchronization even when the requested key is absent, so a startup settings read can propagate an empty reset. This is logical deletion: old journal pages and the older SD slot can remain until overwritten. The reset marker must survive until any disconnected card is synchronized; reinstalling firmware erases the flash marker. Ordinary saves retain the reset generation.

Settings save/reset input consumes both key edges, including release after Alt is released. Status notifications use the existing direct LCD margin renderer and expire after three seconds. Saving performs two checked brightness reads; normal frame rendering adds no settings-storage polling.
...
## Title/demo heap lifetime fixes

Two independent leaks accumulated during repeated attract-mode cycles. The portable sprite loader allocated a temporary DAT palette/header (`shpl`) and never released it after copying the palette into the loaded images and global palette. It now frees that temporary block after `set_loaded_palette()`. The game restart path also discarded `peels_count` without freeing the saved sprite-background surfaces; it now calls `free_peels()` before the restart's `longjmp` can abandon those allocations.

Neither fix enlarges the 164 KiB arena or resets it wholesale. Persistent game assets remain owned by their existing objects. The upstream restart change is included in `src/game/upstream-changes.patch`. The host soak scenario allows two simulated hours, runs 150,000 frames and compares heap usage at matching title phases after warm-up. It catches arena leaks that host ASan cannot detect because the arena itself is a static array.

With the test asset set, the corrected soak completed 150,000 frames (about 77 simulated minutes), observed 61 title entries and held both warm title checkpoints at 74,384 bytes. The arena peak was 154,480 bytes, with zero mixer errors. This is a host regression result, not an on-device endurance measurement.
