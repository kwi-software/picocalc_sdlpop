...
static fat32_error_t write_cluster_fat_entry(uint32_t cluster, uint32_t value)
{
    ...
    *(uint32_t *)(sector_buffer + entry_offset) |= value & 0x0FFFFFFF;
    ...
    // Keep all FAT copies consistent (active-only FAT volumes are rejected).
    for(unsigned fat=0;fat<boot_sector.num_fats;fat++)
        RETURN_ON_ERROR(write_sector(fat_sector+fat*boot_sector.fat_size_32, sector_buffer));
...
{
    uint32_t start=fsinfo.next_free;
    if(start<2 || start>=cluster_count+2)start=2;
    for(uint32_t n=0;n<cluster_count;n++) {
        uint32_t i=2+(start-2+n)%cluster_count,value;
        RETURN_ON_ERROR(read_cluster_fat_entry(i,&value));
        if(value==FAT32_FAT_ENTRY_FREE){*cluster=i;return FAT32_OK;}
    }
...
fat32_error_t fat32_mount(void)
{
    ...
    if (!is_sector_boot_sector(sector_buffer) && is_sector_mbr(sector_buffer))
    {
    ...
    if (boot_sector.total_sectors_32 <= first_data_sector ||
        boot_sector.root_entries != 0 || boot_sector.fat32_version != 0 ||
        (boot_sector.ext_flags & 0x80)) return FAT32_ERROR_INVALID_FORMAT;
    ...
    if (boot_sector.root_cluster < 2 || boot_sector.root_cluster >= cluster_count+2 ||
        (uint64_t)(cluster_count+2)*4 > (uint64_t)boot_sector.fat_size_32*512)
        return FAT32_ERROR_INVALID_FORMAT;
...
static inline uint16_t utf8_to_lfn_ch(const char *name, unsigned index, unsigned len)
{
...
static fat32_error_t find_entry(fat32_entry_t *dir_entry, const char *path)
{
    ...
    while (token)
    {
        ...
        bool found = false;
        fat32_entry_t entry;
        fat32_error_t read_status;
        while ((read_status=fat32_dir_read(&dir, &entry)) == FAT32_OK && entry.filename[0])
        {
        ...
        fat32_close(&dir);
        if(read_status!=FAT32_OK)return read_status;
        if (!found && next_token)
        {
...
static fat32_error_t link_entry(fat32_entry_t *entry, const char *path)
{
    ...
    while (!found)
    {
        ...
        for (uint32_t i = 0; i < FAT32_SECTOR_SIZE; i += 32)
        {
            ...
            if ((uint8_t)entry_ptr->shortname[0] == FAT32_DIR_ENTRY_FREE || entry_ptr->shortname[0] == FAT32_DIR_ENTRY_END_MARKER)
            {
                if (free_count == 0)
                {
    ...
    for (int i = 0; i < needed_entries; i++)
    {
        ...
        uint32_t entry_offset = (free_entry_pos % bytes_per_cluster) + (i * 32);
        uint32_t entry_cluster_offset = entry_offset;
    ...
    uint32_t raw_offset = (free_entry_pos % bytes_per_cluster) + (needed_entries * 32);
    uint32_t entry_cluster;
    CLOSE_AND_RETURN_ON_ERROR(seek_to_cluster(free_entry_cluster,raw_offset/bytes_per_cluster,&entry_cluster));
    entry->sector = cluster_to_sector(entry_cluster) + ((raw_offset % bytes_per_cluster) / FAT32_SECTOR_SIZE);
...
fat32_error_t fat32_write(fat32_file_t *file, const void *buffer, size_t size, size_t *bytes_written)
{
    ...
    if (file->attributes & FAT32_ATTR_READ_ONLY) return FAT32_ERROR_WRITE_FAILED;
    if (file->attributes & FAT32_ATTR_DIRECTORY)
    {
...
void fat32_init(void)
{
    ...
    // All card detection and I/O run synchronously on core 0, never in an IRQ.
