/* SD-first policy; only failed SD writes commit to flash. Core 0 only. */
#include "pc_store_internal.h"
#include <string.h>
static PC_StoreBackend backend;
PC_StoreBackend PC_StoreLastBackend(void) {return backend;}
static size_t length(unsigned key) {return key==PC_STORE_HOF?PC_STORE_HOF_SIZE:key==PC_STORE_SAVE?PC_STORE_SAVE_SIZE:0;}
static unsigned location(unsigned key) {return key==PC_STORE_HOF?16:16+PC_STORE_HOF_SIZE;}
static unsigned token_offset(unsigned key) {return key==PC_STORE_HOF?12:200;}
...
bool PC_StoreRead(unsigned key,void *out,size_t bytes) {
    ...
    if(!load(record) || !(pc_record_get32(record+8)&(1u<<key)))return false;
...
bool PC_StoreWrite(unsigned key,const void *data,size_t bytes) {
    ...
    if(load(record)) {
        ...
        pc_record_put32(record+12,0);pc_record_put32(record+200,0);
    }
