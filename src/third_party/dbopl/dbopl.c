 ...
 * DOSBox DBOPL, C port from Chocolate Doom 2.2.0 (opl/dbopl.c and dbopl.h).
 * https://github.com/chocolate-doom/chocolate-doom/tree/chocolate-doom-2.2.0/opl
 * Local changes: streaming Chip__Reset, portable offsetof calculations and
 * header guard. Original copyright and license notices follow unchanged.
 */
/*
...
#include <math.h>
#include <stddef.h> /* portable offsetof, no null-pointer member access */
#include <stdlib.h>
#include <string.h>
//#include "dosbox.h"
#include "dbopl.h"
...
/* Added for streaming playback: retain expensive sample-rate tables on restart. */
void Chip__Reset(Chip *self) {
    Bit32u i, lfoAdd = self->lfoAdd, noiseAdd = self->noiseAdd;
    memset(self, 0, offsetof(Chip, freqMul));
    memset((unsigned char *)self + offsetof(Chip, chan), 0,
           sizeof(*self) - offsetof(Chip, chan));
    Chip__Chip(self);
    self->lfoAdd = lfoAdd;
    self->noiseAdd = noiseAdd;
    self->noiseValue = 1;
...
void DBOPL_InitTables( void ) {
	...
	{ // haleyjd 09/09/10: Driving me #$%^@ insane
		/* Offset calculation uses offsetof instead of a null instance. */
		for ( i = 0; i < 32; i++ ) {
			Bitu index = i & 0xf;
		...
		for ( i = 0; i < 64; i++ ) {
			...
			Bitu opNum;
			Bitu blah;
			/* No temporary Channel instance required. */
			if ( i % 8 >= 6 || ( (i / 8) % 4 == 3 ) ) {
				OpOffsetTable[i] = 0;
				continue;
			}
